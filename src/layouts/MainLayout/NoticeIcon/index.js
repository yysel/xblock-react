  ...
  getNotificationBox () {
      ...
      return (
        <TabPane tab={title} key={child.props.title}>
          <List
            ...
            onClear={() => this.props.onClear(child.props.type)}
    ...
    return (
      <Spin spinning={loading} delay={0}>
        <Tabs className='xblock-global-header-notice-icon-tabs' onChange={this.onTabChange}>
          {panes}
        </Tabs>
      </Spin>
    )
  ...
  render () {
    ...
    const trigger = (
      <span className={noticeButtonClass}>
        <Badge count={count}>
          <BellOutlined className='xblock-global-header-notice-icon'/>
        </Badge>
      </span>
    )
    if (!notificationBox) {
    ...
    return (
      <Popover
        ...
        popupClassName='xblock-global-header-notice-popover'
        trigger="click"
        arrowPointAtCenter
