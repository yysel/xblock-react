  ...
  render () {
    ...
    const TabBlock=<Fragment>
        <div style={{backgroundColor:'#ffffff',padding:'0 20px'}} >
            ...
            <Tabs onChange={(v)=>this.setState({key:v})} size={"large"} defaultActiveKey={block?.[0]}  tabBarStyle={{margin:0}} >
                {block.map((item) =>  <TabPane  tab={this.state.title?.[item]?this.state.title?.[item]:'加载中...'} key={item} /> )}
                <BackTop>
        ...
        <div  style={{margin:"20px"}}>
            {block.map((item, sequence) => <div   key={item} style={{display:this.state.key===item?'block':'none'}}><Query key={item} sequence={sequence} index={item} {...rest} setTitle={(v)=>{
                const obj={}
                obj[item]=v
